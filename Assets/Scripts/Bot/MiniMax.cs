...
public static class MiniMax
{
    ...
    static int MoveScore(Move move, int ply, Move ttMove)
    {
        ...
        if (move.CaptureMove)
        {
            int victim = PieceValue(
                Position.Squares[move.TargetSquare]
            );
    ...
    static int Search(int depth, int alpha, int beta, int ply)
    {
        ...
        // No legal moves means either:
        // 1. Checkmate
        // 2. Stalemate
        if (moves.Count == 0)
        {
            if (Position.IsKingInCheck(Position.colorToMove))
            {
                // Current player has been checkmated.
                return -MATE_SCORE;
            }
        ...
        moves.Sort((a, b) =>
            MoveScore(b, ply, ttMove).CompareTo(
            MoveScore(a, ply, ttMove)
        ));
    ...
    {
        bool inCheck =
            Position.IsKingInCheck(Position.colorToMove);
        ...
        // If we are in check, we MUST consider
        // every legal move, not just captures.
        if (inCheck)
        {
        ...
        moves.RemoveAll(
            m => !m.CaptureMove && !m.IsPromotion
        );

        moves.Sort((a, b) =>
            MoveScore(b, 0, default).CompareTo(
            MoveScore(a, 0, default)
        ));
    ...
    public static Move GetBestMove(int depth)
    {
        ...
        var (_, policy) =
            ChessEvaluator.EvaluateWithPolicy();
        ...
        // NN policy + normal move ordering.
        moves.Sort((a, b) =>
        {
            float scoreA =
                ChessEvaluator.GetMoveScore(policy, a) * 100f
                + MoveScore(a, 0, ttMove);

            float scoreB =
                ChessEvaluator.GetMoveScore(policy, b) * 100f
                + MoveScore(b, 0, ttMove);
